...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Iván Castro Martínez");
...
struct assoofs_inode_info *assoofs_get_inode_info(struct super_block *sb,
					  uint64_t inode_no)
{
    ...
    printk(KERN_INFO "[assoofs_get_inode_info] > Call starts. Trying to obtain inode no. %lld.", inode_no);
    ...
    printk(KERN_INFO "[assoofs_get_inode_info] > Call finished. Inode %lld obtained.", inode_buffer -> inode_no);
...
/* Función que creará un directorio */
    //param1 inodo del directorio donde se pretende crear el directorio
    //param2 entrada en el directorio padre del nuevo directorio
    //param3 parametro permisos
static int assoofs_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode) {
...
//Funcion para obtener un puntero a la información del inodo pedido
struct assoofs_inode_info *assoofs_inode_search(struct super_block *sb, struct assoofs_inode_info *start, struct assoofs_inode_info *buscar) {
	uint64_t count = 0;
...
/* Almacena el inodo modificado (actualizar la información persistente del inode) */
int assoofs_inode_save(struct super_block *sb, struct assoofs_inode_info *assoofs_inode) {
	struct assoofs_inode_info *inode_iterator;
	struct buffer_head *bh;
    ...
    //obtenemos de disco el almacén de inodes
	bh = sb_bread(sb, ASSOOFS_INODESTORE_BLOCK_NUMBER);
	BUG_ON(!bh);

    //buscamos los datos de inode_info en el almacen usando la funcion search
	inode_iterator = assoofs_inode_search(sb, (struct assoofs_inode_info *)bh -> b_data, assoofs_inode);

    //actualizar el inode, marcamos el bloque como sucio y sincronizamos
...
//permitirá obtener un puntero al inodo numero ino del sb
static struct inode *assoofs_get_inode(struct super_block *sb, int ino) {
	struct inode *inode;
	struct assoofs_inode_info *assoofs_inode;

    printk(KERN_INFO "[assoofs_get_inode] > Call stats. Trying to get the inode no. %lu.", ino);

	assoofs_inode = assoofs_get_inode_info(sb, ino);
    //creamos un inode y le asignamos los campos del nuevo inode
	inode = new_inode(sb);
	inode -> i_ino = ino;
	inode -> i_sb = sb;
	inode -> i_op = &assoofs_inode_ops;
    //para asignar i_fop comprobamos si es un fichero o directorio
	if (S_ISDIR(assoofs_inode -> mode)) {
		inode -> i_fop = &assoofs_dir_operations;
        printk(KERN_INFO "[assoofs_get_inode] > The inode no. %lu is a DIR.", ino);
	} else if (S_ISREG(assoofs_inode -> mode)) {
		inode -> i_fop = &assoofs_file_operations;
        printk(KERN_INFO "[assoofs_get_inode] > The inode no. %lu is a FILE.", ino);
	} else
		printk(KERN_ERR "[assoofs_get_inode] > Unknown inode type. Neither a DIR nor a FILE.");

	/* Deberíamos actualizar la información de acceso al fichero o directorio */
	inode->i_atime=inode->i_mtime = inode->i_ctime = CURRENT_TIME;
	inode -> i_private = assoofs_inode;

    printk(KERN_INFO "[assoofs_get_inode] > Call finished. We have the inode no. %lu.", ino);
...
/*lookup se utiliza para recorrer y mantener el arbol de inodos*/
struct dentry *assoofs_lookup(struct inode *parent_inode, struct dentry *child_dentry, unsigned int flags) {
	printk(KERN_INFO "[assoofs_lookup] > Call starts. It will look for %s FILE/DIR.", child_dentry -> d_name.name);
    
    //accedemos al bloque de disco con el contenido del directorio apuntado por parent_inode
    ...
    //recorremos el contenido del directorio buscando la entrada que se corresponda con el buscado
    //si se encuentra, se crea el inodo
	record = (struct assoofs_dir_record_entry *) bh -> b_data;
    printk(KERN_INFO "[assoofs_lookup] > Looking for %s FILE/DIR.", child_dentry -> d_name.name);
	for (i = 0; i < parent -> dir_children_count; i++) {
		if (!strcmp(record -> filename, child_dentry -> d_name.name)) {
            //funcion auxiliar que obtiene la informacion de un inodo a partir de su nº
...
static int assoofs_create_fs_object(struct inode *dir, struct dentry *dentry, umode_t mode) {
	...
	/* Obtenemos un bloque libre que lo gestionará la siguiente funcion */
    ...
    //esta funcion guardará la informacion persistente del nuevo inodo en el almacen de inodos
	assoofs_inode_add(sb, assoofs_inode);

    //modificamos el contenido del directorio padre añadiendo una nueva entrada para el archivo o directorio
    ...
    //actualizar la información persistente del inodo padre indicando que tiene un archivo mas
    //assoofs_inode_save recorre el almacen y localiza el inodo
